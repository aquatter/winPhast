...
uses
  Windows, Messages, SysUtils, Classes, Controls, directShow9, ActiveX, Graphics, Dialogs, t666;
...
type
  ...
  TArrFrame = array[0..100] of TFrame;
  PFrame = ^TArrFrame;
  THistArr =array [0..255] of Integer;
  PHistArr = ^THistArr;


 //Класс для реализации callback процедуры при использовании Sample Grabber-а
...
//    Frames: PFrame;
  ...
  public
    ...
    procedure Capture;
    procedure GetCapturedBitmaps(p: array of PMyInfernalType);
...
begin
//  FreeMem(Frames, FFrameCount*sizeof(Frames));
  SetLength(Frames, 0)
end;
//------------------------------------------------------------------------------
...
begin
  ...
  if Cnt > 0 then
    begin
      ...
      CopyMemory(Buf, pBuffer, BufferLen);}
      Dec(Cnt);
      if Cnt = 0 then
...
begin
  //GetMem(Frames, FrameCount*sizeof(TFrame));
...
var
  aSampleGrabber, aRender:IBaseFilter;
...
begin
 ...
 // FreeMem(cb.Frames, sizeof(TFrame)*cb.FFrameCount);

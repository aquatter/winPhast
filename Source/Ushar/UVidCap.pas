...
begin
  ...
  InitDriver;
  if VidCapParam.Mode = 0 then  // режим фурье
  begin
    {port:=InPort(VidCapParam.Port+1);
    if (port and 16) <> 0 then
    begin
      ShowMessage('LPT Device not found!');
      _err:=erLPT;
      exit;
    end;}
  end;
  ...
  begin
    LPTWatcher:=TLPTWatcher2.Create(true);
    LPTWatcher.Priority:=tpTimeCritical;
    LPTWatcher.adr:=VidCapParam.Port+1;
    LPTWatcher.FreeOnTerminate:=true;
    LPTWatcher.VidCap:=VidCap;
    SetLedBright(VidCapParam.CameraPort, VidCapParam.LedBright);
  end;
  if VidCapParam.Mode = 1 then
  begin
  ...
  AutoReg:=TAutoReg.Create(true);
  AutoReg.FreeOnTerminate:=true;
  AutoRegTerminated:=false;
  AutoReg.OnTerminate:=AutoRegThreadTerminate;
  ...
  Timer.OnTimer:=OnTimer;}
    ...
    begin
      ...
      VideoMode:=$10;
      VideoSource:=VidCapParam.VideoSource;
    end;
   }
